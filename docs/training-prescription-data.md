...
Engine `3.4.1` emits recommendation snapshot `1.4.0` with prescription `2.4.0`. `standardGuideline/1.0.0` freezes the goal-aware set/rep/RPE/RIR/rest comparison range before readiness and user override changes. The existing snapshot schema now also permits an optional `rolePrescription` override-change pair so an already-complex prescription can audit top-set and back-off counts/ranges without changing the snapshot version; replay derives the aggregate set count and enclosing rep range. The reader retains `1.0.0`/`2.0.0` through `1.3.0`/`2.3.0` without synthesizing either field. Application exercise/template records may additionally carry bounded `customExerciseProfile/1.0.0`, `executionQualityAssessment`, and an opt-in `standardRoleWorkload`; legacy records load with those facts absent.
