...
The app treats that package as untrusted input. It enforces an 8 MiB actual UTF-8 limit, bounded JSON depth/object width, forbidden executable/prototype keys, the exact `personal-evidence-package/1.0.0` schema/privacy marker, allowlisted collections, nonempty core prescription/score/muscle-score arrays, 1,024-record collection caps, and bounded IDs/names/text/numeric fields. Before persistence it reconciles exercise identities with the already loaded public research taxonomy and constructs the replacement prescription engine. A single IndexedDB replacement occurs only after those checks; runtime engine/data and every dependent analysis/recommendation/muscle cache change only after the write succeeds. Invalid packages preserve the prior state.
