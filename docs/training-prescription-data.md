...
| Unified prescription engine | `3.3.9` | 2026-07-18 | `prescription-engine.js` |
| Historical family-volume ledger | `historical-family-volume/1.0.0` | 2026-07-18 | `programming-family-ledger.js` |
| Explicit custom-muscle mapping | `personal-muscle-mapping/1.0.0` | 2026-07-18 | App exercise `primaryMuscle` / `secondaryMuscle` projection |
...
The broad region layer remains presentation-only; Back, Shoulders, Core, and Glutes are not exercise aliases or exact future-generation targets. Programming families are the dose authority. Historical recalculation returns `taxonomyVersion`, `personalMappingVersion`, `programmingFamilyVersion`, `ledgerVersion`, `familyProjectionStatus`, `familyTotals`, and an executable non-persisting rollback contract. One exercise contributes at most once to each family; direct credit wins over fractional credit, local/isometric fatigue remains additive and separate, and only final totals are rounded. Canonical history requires one taxonomy version. Custom history requires the user-authored Primary/Secondary muscle evidence under `personal-muscle-mapping/1.0.0`; automatic name rules do not qualify. Missing, mixed, or unresolved provenance produces no family dose, while an empty week is a known zero. Recommendation snapshot `1.3.0` and prescription schema `2.3.0` are unchanged and do not persist these derived fields. Existing engine 3.3.4 through 3.3.8 snapshots, checksums, identity lineage, and logged facts remain immutable. No application-data migration or snapshot backfill is required; rollback recalculates immutable records with the prior relationship set, and the unchanged reader accepts compatible earlier exact-target records byte-for-byte.
