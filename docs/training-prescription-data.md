...
| Unified prescription engine | `3.4.0` | 2026-07-21 | `prescription-engine.js` |
...
Engine `3.4.0` emits recommendation snapshot `1.4.0` with prescription `2.4.0`. The only new persisted decision field is `standardGuideline/1.0.0`, which freezes the goal-aware set/rep/RPE/RIR/rest comparison range before readiness and user override changes. The reader retains `1.0.0`/`2.0.0` through `1.3.0`/`2.3.0` without synthesizing this field. Application exercise/template records may additionally carry bounded `customExerciseProfile/1.0.0` and `executionQualityAssessment` values; legacy records load with those facts absent.
