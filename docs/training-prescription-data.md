...
| Unified prescription engine | `3.3.6` | 2026-07-15 | `prescription-engine.js` |
| App prescription / recommendation snapshot schemas | `2.3.0` / `1.3.0` | 2026-07-14 | `prescription-engine.js` and the app prescription JSON Schemas |
...
The broad region/programming-family layer remains a display, scope, aggregation, and reporting projection; Back, Shoulders, Core, and Glutes are not exercise aliases or exact future-generation targets. Explicit callers may still prescribe against any valid positive-credit direct or fractional relationship. The resolver returns `taxonomyVersion` so the caller can explain which relationship set it used, but the value is transient: recommendation snapshot `1.3.0` and prescription schema `2.3.0` are unchanged and do not persist a new taxonomy-version field. Existing engine 3.3.4 and 3.3.5 snapshots, checksums, and identity lineage remain immutable. No application-data migration, snapshot backfill, or schema rollback is required; the unchanged reader accepts schema-compatible exact-target records from engine 3.3.6 byte-for-byte.
