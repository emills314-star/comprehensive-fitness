...
The app is a tested local-first PWA/Capacitor workout tracker with templates, active logging, submission/history/editing, progress/volume/fatigue analytics, recovery guidance, versioned evidence-backed prescriptions, private aggregate import, and optional Web Push/workout mutation upload. Accepted foundations now include research database 3.0.0, taxonomy 2.1.0, hardened backend lifecycle contracts, strict transactional imports, separate default-off workout-upload consent, exact-version timer cancellation, explicit resumable remote installation deletion, public CI/release/privacy gates, protected Lift/Dashboard baselines, and the application accessibility/design contract for focus, dialogs, target sizing, reduced motion, forced colors, and exact 320 px/200% reflow. Direct wearable sync, comprehensive nutrition logging, account/cloud restore, automatic unsubscribe/delete orchestration across every disable/reset/clear path, physical screen-reader/native-device acceptance, and production verification are incomplete or unverified.
...
| Explicit workout lifecycle and history integrity | **COMPLETED** / High | Active/cancel/submit confirmation, submitted-only analytics, edit confirmation, active-draft recovery, shared tab/Back exit guard, and pagehide/reload protection against persisting an unconfirmed history transaction. Startup ranks valid IndexedDB/fallback state by revision and promotes before cleanup. Edit entry persists the complete stable pre-edit snapshot; concurrent navigation or mutation aborts stale startup without rollback. External service-worker activation is deferred until Save/Discard plus explicit Update, whose persistence gate refuses a data-losing reload. | `index.html`, accessibility browser regression, app-integration contracts, safety/domain/performance tests; UI/Architecture. |
...
| Canonical exercise–muscle taxonomy and programming families | **COMPLETED** / High | Taxonomy 2.1.0 classifies all 62 canonical exercises through 151 relationships, retains 23 stable anatomical IDs, and projects them into 20 guided accounting families. The guided ledger selects the strongest qualifying relationship once, keeps local fatigue additive, and rounds only final exposed values. Stable IDs now use explicit `rex_00001`–`rex_00753`, `chg_0004` `rex_00754`–`rex_00765`, and `chg_0005` `rex_00766`–`rex_00827` epochs plus a fail-closed full source registry. Recommendation/historical family integration remains **PLANNED / NEEDS REVIEW**. | Taxonomy source/exports, stable-ID fixture and source registry, guided planner/volume integration, taxonomy tests; Project/Architecture/Decision Engine. |
| Backend notification/sync lifecycle hardening | **COMPLETED** / High | Installation-scoped timer IDs, timer versions, delivery claims, tombstones, bounded resumable deletion, push-origin allowlisting, TTLs, and write-sync conflict/idempotency bounds are implemented server-side. The frontend reconciles cancel-before-schedule completion with authoritative IDs, durably records ambiguous outcomes, checks both push IDs by exact version, preserves deletion/timer authorization until local clearing is safe, and reconciles failed-write identity journals without losing a pending bearer or resurrecting it after newer terminal deletion. Automatic remote deletion across every ordinary disable/reset/clear path and restore remain **NEEDS REVIEW**. | `api/`, `index.html`, `sw.js`, backend race/privacy, lifecycle-interleaving, and app-integration tests, `docs/push-backend.md`; Project/Architecture. |
| Private-data import and upload-consent hardening | **COMPLETED** / High | Backup/private-evidence files have exact 8 MiB, shape, allowlist, ID/text/count, duplicate/reference, schema/privacy, and three-source identity-reconciliation gates. Replacement is single-write/transactional; invalid new score, prescription, or muscle-score identities preserve prior persistence, runtime, revisions, and caches. Persisted legacy invalid identities remain auditable but are quarantined from all executable consumers. Hosted private-path discovery is disabled. Workout-mutation queue activity requires separate default-off consent plus immediate epoch invalidation, upload abort, and durable revocation clearing. | `index.html`, privacy/security static and Playwright lifecycle contracts; Project/Architecture/UI/privacy/training map. |
...
- **Current implementation:** **IMPLEMENTED:** the backend defines record-specific TTLs, a global registry cleared by completed deletion, immediate revocation, resumable deletion, idempotency, and revision conflicts. The frontend separates notification permission from default-off epoch-guarded workout-upload consent, reconciles exact-version cancel/schedule interleavings, offers an explicit remote-delete action that continues 202 cleanup across retry/reload/online recovery, and blocks local clearing while deletion or authenticated active-timer cancellation is unresolved. **PLANNED / NEEDS REVIEW:** notification-disable/reset and ordinary local-clear flows do not automatically initiate terminal remote deletion; the product still needs an explicit restore/account decision and must retain the irreducible in-flight delivery disclosure.
...
10. Intentional review of the two stale ordinary Settings screenshots after the cloud-upload consent UI; protected Lift/Dashboard baselines are current and unchanged.
