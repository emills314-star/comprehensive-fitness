...
| Hard-capped practical mesocycle construction | **COMPLETED** / High | Construction enforces 18 working sets/day and two exercises/muscle/day, prioritizes major direct work, uses taxonomy 2.0 direct/fractional/fatigue semantics, reports schedule capacity, hides non-actionable review noise, and offers constrained regeneration. | Engine 3.0.0, planner UI, prescription/integration/hosted browser tests; Decision Engine/Architecture/UI. |
| Canonical exercise–muscle taxonomy and programming families | **COMPLETED** / High | Taxonomy 2.1.0 classifies all 62 canonical exercises through 151 relationships, retains 23 stable anatomical IDs, and projects them into 20 accounting families without rewriting history. Family volume selects the strongest qualifying relationship once, local fatigue remains additive, and only final display exposure is rounded. | Taxonomy source/exports, stable-ID fixture, engine/planner/volume integration, private analysis adapter, taxonomy tests; Project/Architecture/Decision Engine. |
| Backend notification/sync lifecycle hardening | **COMPLETED** / High | Installation-scoped timer IDs, timer versions, delivery claims, tombstones, bounded resumable deletion, push-origin allowlisting, TTLs, and write-sync conflict/idempotency bounds are implemented server-side. **PLANNED / NEEDS REVIEW:** complete frontend cancel/unsubscribe/delete orchestration and restore remain outstanding. | `api/`, backend race/privacy tests, `docs/push-backend.md`; Project/Architecture. |
| Public CI, privacy, and release gates | **COMPLETED** / High | Pull-request/`main` public jobs separate public from ignored private tests, pin Node 22.23.1/npm 10.9.8 and actions by immutable SHA, enforce privacy/workflow/dependency policies, validate research/PWA output, and run Chromium UI audit. The release is currently blocked by the stable-ID regression recorded below, not by missing gate wiring. | `package.json`, `.github/workflows/`, `scripts/run-*`, workflow/privacy/release tests; Architecture. |
...
- **Current implementation:** **IMPLEMENTED:** the backend defines bounded retention, immediate revocation, resumable deletion, idempotency, and revision conflicts. **PLANNED / NEEDS REVIEW:** notification-disable/local-clear/reset flows must explicitly cancel active timers, unsubscribe Web Push, continue installation deletion through terminal status, and explain the irreducible in-flight delivery boundary.
...
- **Stable-ID append-epoch test — NEEDS REVIEW / High:** the 2.0.0 digest fixture still protects 1,756 historical relationship identities, but `scripts/test-taxonomy-stable-ids.js` permits only the 2.1.0 Cable Woodchop append and now fails on declared research 3.0.0 rule mappings (`rex_00754` is the first observed failure). Advance the append-epoch contract without weakening historical ID-to-semantics protection.
