...
| External application-runtime boundary | **COMPLETED** / High | The 973 KB application runtime moved byte-preservingly from inline HTML to owned `app.js`; the shell, CSP, service-worker allowlist, public/native packaging, static contracts, and root/`www` parity now enforce the boundary. This removes the deploy-byte CSP failure class and isolates document changes from executable runtime caching. | `index.html`, `app.js`, contract-source adapter, CSP/deployment/PWA/service-worker tests; Architecture and work log. |
