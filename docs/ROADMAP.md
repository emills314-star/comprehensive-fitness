...
| Compact Today workout logger | **COMPLETED** / High | The active workout is a lower-height single column with inline deload/resistance controls, a collapsible resistance selector, field-aligned prior reps/load/RPE, collapsible progression criteria, compact 44 px set/template actions, and no duplicate Workout Board or bottom recovery-adjustment section. Static density/safety contracts and an interactive browser fixture verify the new hierarchy. | `app-views.js`, `index.html`, workout density/history tests; UI/Architecture/work log. |
| Continuous workout and systemic Strong prior-history recovery | **COMPLETED** / High | Active workouts retain every exercise in one user-scrolled document and set/rest transitions never move the viewport. One indexed, resistance-aware resolver supplies dated prior working sets from the full submitted archive, with exact Strong-name recovery for identity-version drift and ordered-position fallback for role differences. Strong-derived templates retain performance identity/provenance; exact Strong history converts only audited non-safety identity/target gaps into a clearly labelled, editable history target. The private 149-identity inventory passed 149/149 locally without entering public artifacts. Import completion verifies dated history, usable structure, and startability; add/remove set edits persist through reload. | `app-foundation.js`, `app-workout.js`, `app-views.js`, `app-import.js`, Strong fallback browser regression, prescription/domain/safety tests; Architecture/Decision Engine/UI. |
| Family-aware recommendation history | **COMPLETED** / High | One shared versioned ledger now governs guided and historical family projection; recommendation history and weekly analytics expose relationship provenance and family totals, fail closed on missing/mixed versions, preserve immutable records, support recalculation rollback without migration, resist conflicting supplied families, and round only final aggregates. | `programming-family-ledger.js`, engine 3.3.8, app weekly analytics, family/provenance/rollback tests; Architecture, Decision Engine, taxonomy/data maps. |
| Accessibility, focus, and large-text design contract | **COMPLETED** / High | Initial load does not steal focus; explicit four-destination navigation and a keyboard skip link enter main; quick templates retain native button roles; modal sheets verify/fallback initial focus, trap, and durably restore focus; Progress nested Back restores context; Equipment/Scope groups are named with 44 px targets; Today actions are contextual; reduced motion, forced colors, and exact 320 px/200% reflow pass with intentionally refreshed protected goldens. | `index.html`, `tests/ui/accessibility-design-regressions.spec.js`, app-integration focus cases, protected-surface suite; UI/Architecture/design specification. |
| Unified prescription snapshot across surfaces | **COMPLETED** / High | Template, coach, chart, live workout, and history share versioned recommendations. Identity resolution separates exact performance history from optional research taxonomy, accepts collision-free reconciled legacy personal IDs, preserves named/equipment variations, and gives direct public identity precedence over name collisions. Typed identity/target failures remain zero-execution; submitted history stays immutable; executable reuse still requires schema/checksum, identity, target, and host binding. | `prescription-engine.js`, application segments, root `schemas/`, canonical-adapter/app-integration/browser lifecycle tests; Architecture/Decision Engine/taxonomy report. |
...
| Canonical exercise–muscle taxonomy and programming families | **COMPLETED** / High | Taxonomy 2.1.0 classifies all 62 canonical exercises through 151 relationships, retains 23 stable anatomical IDs, and projects them into 20 shared accounting families. Guided and historical ledgers select the strongest qualifying relationship once, keep local fatigue additive, round only final exposed values, and fail closed when historical provenance is unverifiable. Future frontend generation consumes the exact identity/default-target adapter without migrating historical snapshots. Stable IDs use explicit append epochs plus a fail-closed full source registry. | Taxonomy source/exports, shared family ledger, stable-ID fixture and source registry, canonical adapter, frontend lifecycle, guided/recommendation volume integration, taxonomy tests; Project/Architecture/Decision Engine. |
...
| Deploy-byte CSP integrity | **COMPLETED / SUPERSEDED** / Critical | Hosted verification caught a Windows CRLF/local versus Vercel LF hash mismatch that blocked the application inline runtime. The immediate deploy-byte hash fix restored production; the later external-runtime boundary removed inline hashes and therefore eliminated this failure class. | `.gitattributes`, `vercel.json`, deployment contract, hosted mobile/desktop safety smoke; Architecture and both runtime-boundary work logs. |
| External application-runtime boundary | **COMPLETED** / High | The 973 KB application runtime first moved byte-preservingly from inline HTML to an owned external boundary and is now distributed across the bounded application segments below; the shell, CSP, service-worker allowlist, public/native packaging, static contracts, and root/`www` parity enforce it. This removes the deploy-byte CSP failure class and isolates document changes from executable runtime caching. | `index.html`, application runtime segments, contract-source adapter, CSP/deployment/PWA/service-worker tests; Architecture and work log. |
| Bounded application-runtime ownership | **COMPLETED** / High | The previously concentrated 973 KB runtime is byte-preservingly divided into eight ordered responsibility segments. No segment exceeds 300 KiB; static concatenation/load-order contracts, browser suites, SW v35, CSP, and root/`www` parity protect the boundary. Shared classic-script lexical coupling remains a documented residual risk. | `app-foundation.js` through `app.js`, runtime-boundary/deployment/PWA tests; Architecture, performance, and work log. |
| First independent dual-40 rubric milestone | **COMPLETED** / High | A score-neutral reviewer audited pushed revision `ecb32c5e321a652442dcbf9334df6abc56b0cb2a`: Codebase 40/50 and Workout recommendation 41/50, with no terminal defect reproduced in tested scope. Final frozen gates remain unmet; dose-family historical/recommendation integration is the only category below 4. | Frozen rubric score record and `2026-07-18-first-milestone-independent-score.md`. |
| Durable cloud-consent presentation | **COMPLETED** / High | The cloud-copy checkbox remains canonical and pending during server authorization; enabled state appears only after the explicit IndexedDB consent write succeeds. The browser regression polls durable storage before reload, eliminating a real immediate-reload race without weakening consent assertions. | `app.js`, sync/app-integration contracts, mobile/desktop browser lifecycle; Architecture and runtime-boundary work log. |
| Selectable lively color packages | **COMPLETED** / High | Ten semantic color packages are selectable under More, persist across reload/export/import, preserve separate action/success/warning/destructive roles, and default new or legacy installs to Signal Garden. | `index.html`, settings/persistence/import runtime, mobile/desktop browser tests; UI/UX, Architecture, color-package guide. |
...
| Explicit frontend module imports | **PROPOSED** | Low | Replace the now-bounded ordered classic-script lexical contract with explicit imports only after offline/native compatibility and browser parity are demonstrated. | Application runtime segments; Architecture. |
...
- **Shared runtime lexical environment — Medium:** UI, state, migrations, imports, and integrations now have bounded responsibility files, but the ordered classic scripts still share browser-global lexical state. Runtime load-order, parse, browser, packaging, and cache tests are mandatory.
...
- **COMPLETED (session logger refinement):** Rebuilt 25 synthetic screens for the five shortlisted directions around a shared fast-logging contract: prior performance, set types, direct load/reps editing, completion-triggered rest, session actions, today-only edits, contextual recommendations/warnings, and richer template browsing. Body Atlas was removed from the shortlist; the shared light-blue palette remains verified for Dual Track, Mission Control, and Editorial Performance.
