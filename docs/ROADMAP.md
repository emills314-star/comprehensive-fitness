...
- **Current implementation:** **IMPLEMENTED:** the backend defines record-specific TTLs, a global registry cleared by completed deletion, immediate revocation, resumable deletion, idempotency, and revision conflicts. **PLANNED / NEEDS REVIEW:** notification-disable/local-clear/reset flows must explicitly cancel active timers, unsubscribe Web Push, continue installation deletion through terminal status, and explain the irreducible in-flight delivery boundary.
