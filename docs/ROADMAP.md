...
| Continuous workout and systemic Strong prior-history recovery | **COMPLETED** / High | Active workouts retain every exercise in one user-scrolled document and set/rest transitions never move the viewport. One canonical, resistance-aware resolver supplies dated prior working sets across all exercises; role mismatches fall back by ordered working-set position, Strong-derived templates retain performance identity, unusable zero-set snapshots fall back to saved template structure, and import completion audits all generated exercises. | `app-foundation.js`, `app-analysis.js`, `app-workout.js`, `app-views.js`, `app-import.js`, prescription/domain/safety tests; Architecture/Decision Engine/UI. |
