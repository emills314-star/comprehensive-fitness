...
| Deploy-byte CSP integrity | **COMPLETED** / Critical | Hosted verification caught a Windows CRLF/local versus Vercel LF hash mismatch that blocked the application inline runtime. HTML is now LF-pinned, the contract hashes deploy-equivalent bytes, and the production CSP hash is verified against the served script. | `.gitattributes`, `vercel.json`, deployment contract, hosted mobile/desktop safety smoke; Architecture and work log. |
