...
| Backend notification/sync lifecycle hardening | **COMPLETED** / High | Installation-scoped timer IDs, timer versions, delivery claims, tombstones, bounded resumable deletion, push-origin allowlisting, TTLs, and write-sync conflict/idempotency bounds are implemented server-side. The frontend now performs exact-version cancellation and explicit resumable deletion; automatic unsubscribe/delete across every disable/reset/clear path and restore remain **NEEDS REVIEW**. | `api/`, `index.html`, `sw.js`, backend race/privacy and app-integration tests, `docs/push-backend.md`; Project/Architecture. |
| Private-data import and upload-consent hardening | **COMPLETED** / High | Backup/private-evidence files have exact 8 MiB, shape, allowlist, ID/text/count, duplicate/reference, schema/privacy, and engine-reconciliation gates; replacement is single-write/transactional and rejected imports preserve prior state. Hosted private-path discovery is disabled. Workout-mutation queue reads/writes require separate explicit consent that defaults off. | `index.html`, privacy/security static and Playwright contracts; Project/Architecture/UI/privacy/training map. |
...
- **Current implementation:** **IMPLEMENTED:** the backend defines record-specific TTLs, a global registry cleared by completed deletion, immediate revocation, resumable deletion, idempotency, and revision conflicts. The frontend separates notification permission from default-off workout-upload consent, cancels exact timer versions, and offers an explicit remote-delete action that continues 202 cleanup across retry/reload/online recovery. **PLANNED / NEEDS REVIEW:** notification-disable/local-clear/reset flows must automatically cancel active timers, unsubscribe Web Push, and initiate terminal deletion where promised; the product still needs an explicit restore/account decision and must retain the irreducible in-flight delivery disclosure.
