...
| Public-only web/native packaging boundary | **COMPLETED** / Critical | Explicit asset allowlist, contained stale-private-payload pruning across web/Android/iOS roots, package/source hash parity, sensitive-artifact scanning, disabled Android backup/transfer and cleartext traffic, and narrowly scoped FileProvider paths pass automated checks. Personal evidence remains a user-controlled local import. | `scripts/sync-web.ps1`, `scripts/verify-pwa.ps1`, Android manifest/XML, native-privacy tests; Architecture/training-data docs. |
| Public-only offline cache boundary | **COMPLETED** / High | Immutable same-origin shell allowlist, sensitive/unlisted/query/cross-origin exclusions, no-store/private response rejection, constrained offline navigation, safe notification targets, cache-version retirement, and adversarial cache-policy tests pass. | `sw.js`, service-worker-cache tests, PWA verifier; Architecture. |
| Conditional set-load progression messaging | **COMPLETED** / High | Top/back-off/working-set load increments are gated by the complete role-level rep/RPE requirement; unqualified back-off cards hold current load and distinguish conditional progression from evidence confidence; regression coverage passes. | `index.html:setPrescriptionForRole`, `scripts/test-set-prescriptions.js`; Decision Engine/UI. |
...
| Canonical exercise–muscle taxonomy and unique mesocycle exercises | **COMPLETED** / High | Database/taxonomy 2.0 classifies all 61 canonical exercises with direct, fractional, incidental, isometric, and unknown semantics; low-confidence work is queued. Engine 3.0 uses canonical IDs once per generated mesocycle, weighted hypertrophy volume, separate fatigue exposure, and deterministic historical recalculation. | Taxonomy source/exports, engine, planner/volume UI, private analysis adapter, tests, hosted verification work log. |
...
- **Duplicated packaged assets — Low:** root and `www/` can drift between copy steps; hash-parity and sensitive-payload checks in mandatory `sync:web`/`verify:pwa` prevent release of stale assets.
