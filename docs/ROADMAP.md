...
| --- | --- | --- | --- |
| User-defined exercise and per-set template workflow | **COMPLETED** / High | Persistent validated `user:` catalog supports create/select/edit/archive/restore, required primary plus optional secondary muscle mapping, resistance/equipment metadata, guided planning, template/workout propagation, family-authoritative analytics, and atomic backup import/export. Template setup supports exact or min/max rep targets for every working set. Actionable coaching uses collapsed `Why This Recommendation` disclosures, custom records disclose limited confidence/no fabricated canonical research, logged-workout Edit is discoverable, and the progression gate repeats prior load/reps until rep/RPE criteria pass. | `app-foundation.js`, `app-analysis.js`, `app-workout.js`, `app-views.js`, `app-import.js`, `backup-contract.js`, `scripts/test-custom-exercise-workflow.js`, set-prescription tests; Project/Architecture/Decision Engine/UI/UX. |
| Family-authoritative weekly recommendation dose | **COMPLETED** / High | One shared versioned ledger governs guided, historical, Dashboard, fatigue-flag, and progression dose. Exact family totals—not broad display buckets—drive weekly recommendation input and status. Canonical history requires one taxonomy version; custom history requires explicit versioned Primary/Secondary muscle evidence; unresolved name inference emits no dose and renders a mapping-required warning. Empty history is a known zero. Mobile/desktop adversarial tests cover mixed canonical/custom totals and the unresolved transition. | `programming-family-ledger.js`, engine 3.3.9, app weekly analytics/UI, family/provenance/rollback/browser tests; Architecture, Decision Engine, UI/UX, taxonomy/data maps. |
