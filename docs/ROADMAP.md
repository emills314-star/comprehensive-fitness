...
| Continuous workout and systemic Strong prior-history recovery | **COMPLETED** / High | Active workouts retain every exercise in one user-scrolled document and set/rest transitions never move the viewport. One canonical, resistance-aware resolver supplies dated prior working sets across all exercises; role mismatches fall back by ordered working-set position. Strong-derived templates retain performance identity/provenance, and an exact submitted Strong identity without a research crosswalk receives a clearly labelled, editable non-research history target instead of a zero-set safety block. Import completion verifies dated history, usable structure, and startability; add/remove set edits persist through reload. | `app-foundation.js`, `app-workout.js`, `app-views.js`, `app-import.js`, Strong fallback browser regression, prescription/domain/safety tests; Architecture/Decision Engine/UI. |
