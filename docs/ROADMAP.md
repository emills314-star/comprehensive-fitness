...
| Explicit workout lifecycle and history integrity | **COMPLETED** / High | Active/cancel/submit confirmation, submitted-only analytics, edit confirmation, active-draft recovery, shared tab/Back exit guard, and pagehide/reload protection against persisting an unconfirmed history transaction. Startup ranks valid IndexedDB/fallback state by revision, canonical content, and reliable persistence time; unorderable divergence preserves both copies through ordinary saves until confirmed Clear All. Edit entry persists the complete stable pre-edit snapshot; concurrent navigation or mutation aborts stale startup without rollback and awaits current-state reconciliation, with truthful dual-store failure guidance. External service-worker activation is deferred until Save/Discard plus explicit Update, whose persistence gate refuses a data-losing reload. | `index.html`, accessibility browser regression, app-integration contracts, safety/domain/performance tests; UI/Architecture. |
