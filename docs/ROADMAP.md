...
The app is a tested local-first PWA/Capacitor workout tracker with templates, active logging, submission/history/editing, progress/volume/fatigue analytics, recovery guidance, versioned evidence-backed prescriptions, private aggregate import, and optional Web Push/workout mutation upload. Accepted foundations now include research database 3.0.0, taxonomy 2.1.0, hardened backend lifecycle contracts, strict transactional imports, separate default-off workout-upload consent, exact-version timer cancellation, explicit resumable remote installation deletion, public CI/release/privacy gates, and dedicated protected Lift/Dashboard baselines. Direct wearable sync, comprehensive nutrition logging, account/cloud restore, automatic unsubscribe/delete orchestration across every disable/reset/clear path, the application work required by the new accessibility/design regression contract, and production/device verification are incomplete or unverified.
...
- **Current implementation:** **IMPLEMENTED:** repository-owned Playwright coverage checks all five primary destinations at mobile/desktop viewports, approved screenshots, axe A/AA, overflow/clipping, console errors, source-style ceilings, documentation presence, and dedicated protected Lift/Dashboard behavior and visual baselines. Pull-request/`main` public CI plus the weekly/manual audit own the established release checks. **PARTIALLY IMPLEMENTED:** deterministic template start through submission/history/chart, reload persistence, unit toggling, full modal-focus behavior, offline transitions, data-heavy edge fixtures, and the newly specified accessibility/design regression matrix still need application completion or release-gate integration.
- **Acceptance criteria:** extend the deterministic suite to cover template start → log → confirm submit → summary → history → chart, reload persistence, responsive visual inspection, console errors, unit toggling, and the pending accessibility/design contract without private fixture data or protected-surface redesign.
