...
This adapter does not broaden or replace explicit caller targets. `prescribeExercise` continues to accept a valid positive-credit fractional relationship and reject an unrelated target. The app now adopts the adapter for future automatic generation: it resolves identity, derives the exact default target, and only then calls the engine. A trusted custom/user identity must be present and valid in the prepared personal reconciliation map; no name-based target is fabricated for an untrusted custom exercise. Any adapter failure becomes a typed zero-set/zero-rep/zero-load hard rejection and never reaches `prescribeExercise`. Existing broad display/reporting projection remains a separate presentation concern.

Submitted historical snapshots are immutable. The frontend returns a snapshot attached to a submitted session before current identity or default-target resolution, with no migration, backfill, checksum refresh, or taxonomy-field injection. Older broad-target history therefore remains byte/reference-equivalent and readable. A stored active/template snapshot is different because it can execute: the frontend reuses it only after schema/checksum, canonical identity, direct-target, and host validation. Failure returns a deterministic non-executable `invalid_stored_recommendation_snapshot` with zero sets/reps/load, performs no fresh prescribe call, and does not cache the rejected object. Engine 3.3.9 exact targets continue to use the existing snapshot/prescription schemas.
...
Active set cards distinguish evidence confidence from progression readiness. A top set must meet its own programmed rep maximum and RPE ceiling before its candidate next load is actionable. A back-off structure must have every programmed comparable back-off set at or above its role-specific rep maximum and at or below its RPE ceiling. Until that gate passes, the execution target repeats the prior comparable load, caps reps at the prior comparable performance, retains the programmed range separately for audit, and labels confidence conditional. This prevents a high-confidence evidence sample from being presented as permission to increase weight or jump reps before the full role-level gate is met (`app-analysis.js:setPrescriptionForRole`, `scripts/test-set-prescriptions.js`).

User-defined exercises use a separate, explicitly limited-confidence path. A saved primary muscle contributes direct programming-family dose and an optional secondary muscle contributes 0.5 fractional dose under `personal-muscle-mapping/1.0.0`. Template set targets and submitted personal performance can guide practical rep/load holds or progressions. The app does not invent canonical effectiveness scores, research citations, exact anatomical taxonomy, or pain-safe substitution evidence for these records. Missing explicit mappings fail closed for recommendation dose.
...
Weekly muscle volume uses Monday–Sunday submitted history. Direct sets count 1; exercise-specific fractional work counts its verified relationship credit; incidental and unknown relationships count zero; isometric exposure affects fatigue context rather than hypertrophy-set totals. Each exercise is first coalesced by programming family so anatomical subdivisions cannot duplicate one exposure; broad UI regions remain presentation-only. Exact family totals now drive `currentWeeklySets`, family target status, and overload flags. Canonical exercises use taxonomy provenance. A custom exercise contributes only when the user saved an explicit Primary muscle (and optional Secondary muscle) under `personal-muscle-mapping/1.0.0`; its name is never treated as verified dose evidence. Any submitted unresolved custom exercise blocks the weekly family projection, passes no weekly dose into progression, and produces a visible mapping-required status. An empty week is a known zero. Warm-ups do not count; explicitly deloaded work is excluded. The result carries taxonomy and personal-mapping provenance, ledger versions, exact family totals, projection status, and a non-persisting rollback contract. Logged workout facts remain unchanged; changing or rolling back evidence recalculates derived values from immutable records. Full precision is retained through aggregation and only final exposed totals are rounded.
