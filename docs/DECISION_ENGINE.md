...
Active set cards distinguish evidence confidence from progression readiness. The recommendation snapshot's `finalPrescription` is the authoritative source for a live set's prescribed role, load, rep target/range, RPE target, and set count. Workout startup converts that final object—not `basePrescription`—into the executable target before generating set rows, and the summary and expanded rationale read the same saved final object. `basePrescription` exists only to explain what changed. Prior comparable performance remains evidence and progression context; it cannot silently replace those executable targets. A top set must meet its own programmed rep maximum and RPE ceiling before its candidate next load is actionable. A back-off structure must have every programmed comparable back-off set at or above its role-specific rep maximum and at or below its RPE ceiling; until then, the UI labels the progression state conditional/hold-load without replacing the current `finalPrescription` load or repetitions. This prevents the recommendation summary, rationale, and editable rows from presenting different prescriptions (`app-foundation.js:coachTargetForTemplateExercise`, `app-analysis.js:setPrescriptionForRole`, `scripts/test-prescription-app-integration.js`, `scripts/test-set-prescriptions.js`).
