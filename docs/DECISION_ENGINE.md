...
The portfolio is placed into the configured training days only after selection. Placement cost considers session duration, repeated movement patterns/muscles, adjacent-session recovery, high-fatigue compound concentration, spinal load, and grip demand. Review aggregates direct and indirect volume, frequency, local fatigue, systemic fatigue, spinal load, grip, duration, and missing exposures. Warnings name the exercises in conflict, explain why it matters, and recommend a resolution. Implemented policy defaults are 75-minute target/100-minute maximum sessions, no more than three high-fatigue compounds per session, maximum session spinal load 180, and grip demand 190. These are operational heuristics.
