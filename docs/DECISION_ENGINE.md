...
Active set cards distinguish evidence confidence from progression readiness. A top set must meet its own programmed rep maximum and RPE ceiling before its candidate next load is actionable. A back-off structure must have every programmed comparable back-off set at or above its role-specific rep maximum and at or below its RPE ceiling; until then, `nextLoad` resolves to the prior comparable load and the target reps are capped at the prior comparable performance. The UI labels the state conditional/hold-load. This prevents a high-confidence evidence sample from being presented as permission to increase weight or jump from (for example) 7 prior reps to 10 at a heavier load before the full role-level gate is met (`index.html:setPrescriptionForRole`, `scripts/test-set-prescriptions.js`).
