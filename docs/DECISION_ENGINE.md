...
Mechanical redundancy requires a known shared primary movement pattern, overlapping primary muscle family, and compatible joint action. The implemented similarity also considers role, loading profile, stability, regional emphasis, and equipment, and requires a score of at least 0.72 before issuing a redundancy warning. Unknown or missing movement patterns score zero for redundancy; the review emits only a low-priority metadata notice and states that no conflict was assumed. Shared equipment, generic compound status, broad push/pull labels, or incidental secondary-muscle overlap cannot by themselves mark exercises redundant; therefore a deadlift is not redundant with bench press, cable crossover, or chin-up. Repeated hinges for the same primary target can trigger review, while press/fly overlap remains a role/resistance-profile decision rather than automatic interchangeability. Systemic, spinal, grip, or joint conflicts remain separate fatigue/recovery findings and are never mislabeled as movement duplication.
