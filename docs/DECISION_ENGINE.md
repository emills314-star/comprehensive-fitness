...
Equipment filtering uses explicit requirement alternatives. Each alternative is an AND-list (for example barbell + plates + bench + rack); alternatives are OR paths (for example dumbbell, or selectorized machine). `All Equipment` bypasses restriction explicitly. Bodyweight alone does not satisfy a pull-up bar, Nordic anchor, ab wheel, cable, machine, dumbbell, or barbell requirement. Missing verified equipment metadata fails closed whenever restrictions are active.
...
Mechanical redundancy requires a known shared primary movement pattern, overlapping primary muscle family, and compatible joint action. Unknown patterns never create redundancy. Shared equipment, generic compound status, or incidental secondary-muscle overlap cannot by themselves mark exercises redundant; therefore a deadlift is not redundant with bench press, cable crossover, or chin-up. Repeated hinges for the same primary target can trigger review, while press/fly overlap remains a role/resistance-profile decision rather than automatic interchangeability.
