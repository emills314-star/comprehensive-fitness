...
- **History-edit persistence, recovery conflicts, and app updates:** Starting an edit first flushes a cloned stable snapshot so an unrelated debounced template or settings change is retained. Startup compares valid IndexedDB and local fallback copies by revision, canonical content, then reliable persistence time. A newer fallback cannot be shadowed by stale readable IndexedDB. Divergent equal-revision copies with equal or unavailable timing are both preserved; Settings shows an always-visible status that explains the conflict, asks for an export, and identifies confirmed **Clear All Local App Data** as the only destructive resolution. Ordinary saves and reloads do not remove that alternate. If navigation or another setting changes while the edit snapshot is being saved, edit mode does not open or roll back the newer state; the app awaits a second save of the current clone. The toast says the current changes were saved only after that succeeds. If both stores fail, it instead says the changes are not durable and tells the user to export or retry before reloading. If a new service-worker controller activates while the edit is starting or active, the app stays in place and the update banner asks the user to Save or Discard. Once the edit is resolved, `Update now` is explicit, saves the current stable state, and refuses to reload if both local persistence layers fail.
