...
Ten livelier color packages are **IMPLEMENTED** and documented in `docs/design/UI_COLOR_PACKAGES.md`. Signal Garden is the default. Users select and persist a package in **More → Appearance and units → Color package**; the selected package changes canvas, cards, action/current, coaching/rest, success, warning, and destructive roles. Two five-screen Strong reference restyle sets remain local under the privacy-ignored `docs/design/strong-ui-mockups/` path because their source screenshots contain personal workout details.
...
5. Completing an eligible set can auto-start that exercise’s rest target. The active rest panel is bounded by its own circular rest icon; the exercise card and set block retain their normal neutral boundaries. Its compact treatment reduces vertical chrome while retaining 44 px touch targets. The notice names the next set and supports dismissal/return.
...
The active Today page is one compact workout column with no parallel Workout Board or bottom recovery-adjustment section. Each exercise places the deload toggle beside a collapsed resistance-type selector. Working sets keep prior reps, load, and RPE directly below their matching editable field. Progression criteria use a collapsed **Progress when** row, and the add-set, add-warm-up, copy-set, and save-template actions use low-chrome controls with 44 px targets. Exercise options, add exercise, workout details, and progression rationale remain available through native disclosures so supporting controls do not dominate page height.
...
Every active working-set card shows **Last time** from the newest eligible submitted workout with its date. Prior reps, load, and RPE are field-aligned beneath the corresponding current input instead of appearing in a separate summary block. This lookup searches the full submitted archive, not only the six-month analytics window. Matching uses stable performance identity first and exact Strong exercise name when identity provenance changed, while retaining resistance type and keeping differently named variations separate. It then prefers the same set role and role index, followed by the same ordered working-set position so imported Strong classification differences do not turn real history into an empty state. A compact dash is used only when no comparable field value exists.
...
| Exercise-specific work and rest timers | **IMPLEMENTED** | Template/exercise `restSeconds`, target set roles, `startTimer`, foreground controller, optional push, and a bounded circular rest icon that does not recolor the full exercise/set boundary. “Work timer” as a separate elapsed exercise timer is **NEEDS REVIEW**. |
