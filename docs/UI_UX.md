...
- **App update during history editing:** Starting an edit first flushes a cloned stable snapshot so an unrelated debounced template or settings change is retained. If a new service-worker controller activates while the edit is starting or active, the app stays in place and the update banner asks the user to Save or Discard. Once the edit is resolved, `Update now` is explicit, saves the current stable state, and refuses to reload if both local persistence layers fail.
