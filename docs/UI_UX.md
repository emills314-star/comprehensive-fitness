...
- **History-edit persistence and app updates:** Starting an edit first flushes a cloned stable snapshot so an unrelated debounced template or settings change is retained. Startup compares the valid IndexedDB and local fallback copies by data revision, so a newer fallback cannot be shadowed by stale readable IndexedDB. If navigation or another setting changes while the edit snapshot is being saved, edit mode does not open or roll back the newer state; an accessible toast asks the user to retry. If a new service-worker controller activates while the edit is starting or active, the app stays in place and the update banner asks the user to Save or Discard. Once the edit is resolved, `Update now` is explicit, saves the current stable state, and refuses to reload if both local persistence layers fail.
