...
Exercise Assignments provides compact jump navigation and a distinct container explaining where each selected exercise contributes. Selected exercises render first; alternates return no markup until `View Alternates` is requested, then identify themselves as replacements for the assignment. Static muscle, role, movement, and session tags always use neutral surfaces; blue is reserved for selected or interactive controls. Candidate `Why the Score?` uses the same expandable pattern as muscle education and reports objective fit, target-muscle fit, equipment compatibility, personal evidence, progression, fatigue, program redundancy, stability, and any known limitations. Full Program Review renders each day in a separate session card with a combined day/purpose header and subordinate exercise rows. Findings are compactly grouped as Blocking Issues, Recommended Changes, Warnings, Informational Notes, and Passed Checks; only the highest-priority group opens by default. Blocking volume, frequency, consecutive-heavy-pattern, exercise-count, or working-set findings prevent activation.

Program Slot summaries use exercise chips, session badges, role/frequency badges, and omit repeated generic slot rationale. `Compare Details` builds a visible comparison of movement, primary muscles, equipment, stability, progression, fatigue, placement, and score limitations. Missing movement/equipment metadata is omitted or labelled `Metadata Review Needed`; the UI never displays an unexplained `Unknown` badge. The weekly muscle summary makes direct sets the primary number, then total contributing sets, target range, sessions, frequency, and below/within/above status. Contributing sets are direct 1.0 plus only mapped meaningful secondary work at 0.5.
