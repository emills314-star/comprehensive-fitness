...
Weekly Dashboard status and recommendation dose are programming-family authoritative. A broad card such as Back or Shoulders is a presentation container whose expanded detail lists each verified family with its own sets, target, direct/fractional breakdown, taxonomy version, and optional `personal-muscle-mapping/1.0.0` provenance. A custom exercise contributes only after the user saves a Primary muscle; an optional Secondary muscle receives fractional credit. If any submitted custom exercise remains unmapped, the Dashboard shows **mapping needed**, emits no family totals, and tells the user to edit that exercise instead of inferring dose from its name. The same blocked state passes no `currentWeeklySets` into progression. A week with no submitted work remains a known zero-dose state. Family-status pills and summaries wrap inside compact cards at narrow widths instead of creating horizontal overflow.
...
The Templates tab also owns the custom exercise library. Create/edit requires an exact non-colliding name, primary muscle, resistance type, and optional secondary muscle/equipment. Archive removes a record from new selection while existing templates and history retain the stable `user:` identity; restore makes it selectable again. Template exercise setup selects either a built-in or custom catalog entry and offers a collapsed per-set target editor. Each working set may use an exact rep target or min/max range, and those values appear on the active set card and progression gate.
...
Workout recommendation detail, Recovery readiness, and every quick-template recommendation use the exact collapsed `Why This Recommendation` heading. The detail shows prior versus today, each material set/load/rep/RPE change, the evidence window, expected duration, and resumption criteria. Recommendation badges describe the actual delta (`Progress Load`, `Add One Rep`, `Hold`, or fatigue reduction), rather than using generic Progress when the prescription is unchanged. User-defined disclosures identify the saved muscle mapping and limited-confidence/no-canonical-research boundary. Set rows keep an unqualified top/back-off progression at the prior comparable load and no more than the prior reps until the full rep/RPE gate passes. Pound fields and displays never expose floating-point conversion residue and use 0.5-lb increments.
