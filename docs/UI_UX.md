...
Only one active workout is allowed. Other starts are disabled with one canonical resume notice (`scripts/test-performance.js`). Confirmed submission is guarded before any calculation with a per-session in-progress lock; double-routed or reentrant confirmation produces one completed workout and one set of effects. Template edits during a workout remain session-specific unless the user explicitly updates the template.

Template sets, reps, target RPE, load increment, and rest fields expose exact HTML min/max/step constraints backed by the same domain model used by patches and imports. Invalid raw input stays visible in its connected field, receives `aria-invalid`/`aria-describedby`, and creates one card-level alert; it is not committed, saved, or erased by an unrelated rerender. Correcting the field clears the alert and creates one logical revision even when the browser delivers `input`, `change`, and `focusout`. Deleting the exercise/template, importing, or clearing local data removes any associated transient draft.
