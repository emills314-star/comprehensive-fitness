...
Every active working-set card shows **Last time** from the newest eligible submitted workout with its date. Matching prefers the same set role and role index, then the same ordered working-set position so imported Strong classification differences do not turn real history into an empty state. Only genuinely absent history uses “No prior working set found.”
