...
Ten livelier non-production color packages are documented in `docs/design/UI_COLOR_PACKAGES.md`. Two five-screen Strong reference restyle sets remain local under the privacy-ignored `docs/design/strong-ui-mockups/` path because their source screenshots contain personal workout details. They are **DESIGN MOCKUPS**, not implemented app themes; selecting one requires a separate implementation and browser/device validation task.
