...
Every active working-set card shows **Last time** from the newest eligible submitted workout with its date. This lookup searches the full submitted archive, not only the six-month analytics window. Matching uses stable performance identity first and exact Strong exercise name when identity provenance changed, while retaining resistance type and keeping differently named variations separate. It then prefers the same set role and role index, followed by the same ordered working-set position so imported Strong classification differences do not turn real history into an empty state. Only genuinely absent history uses “No prior working set found.”
...
Compact rest treatment reduces vertical chrome while retaining 44 px touch targets.
