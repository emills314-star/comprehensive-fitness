...
- The final post-planner-refinement 2026-07-11 large-history browser regression (180 submitted sessions, 1,440 exercises, 5,760 sets) measured the initial Templates frame at **44.4 ms on the 375 px mobile project** and **8.2 ms on desktop Chromium**. `tests/ui/ui-audit.spec.js` enforces a conservative 250 ms ceiling and verifies that collapsed editors and mesocycle candidates are absent from the initial DOM, then verifies editor open/close behavior. Mobile samples vary with evidence initialization but remain well inside the guardrail.
