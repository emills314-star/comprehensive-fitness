...
- **PARTIALLY IMPLEMENTED:** Public CI includes Playwright, axe, responsive/overflow, console, and approved-screenshot checks. Dedicated protected Lift/Dashboard behavior and visual baselines now cover mobile and desktop reference states without redesigning either surface. Physical native-device CI remains absent, and the separate accessibility/design regression contract still requires its application implementation before it can become a release gate.
