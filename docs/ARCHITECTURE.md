...
Mesocycle construction is portfolio-first and priority-ordered: normalized evidence creates traceable pools; program slots derive direct-volume/frequency requirements; major muscles and high-value compounds are selected first; the distributor places each canonical exercise on one training day only while enforcing 18 working sets and two exercises per muscle per day; smaller/accessory work uses remaining capacity. Distinct exercises provide repeated weekly muscle exposures. The review aggregates taxonomy-defined direct sets, 0.5/0.25 fractional contribution, zero-credit incidental/unknown relationships, separate isometric fatigue, duration, overlap, frequency, and capacity conflicts. Templates consume these planned sessions. Candidate selection and regeneration rerun the same deterministic constraints rather than randomizing around warnings.
