...
- Vercel CSP hashes are calculated from deploy-equivalent LF-normalized HTML bytes. Repository attributes pin HTML to LF, and the deployment contract normalizes a Windows CRLF checkout before hashing so the browser receives a matching executable-script policy.
