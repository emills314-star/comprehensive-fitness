...
Workout-mutation upload has a separate persisted user consent that defaults off. Enabling is committed locally only after installation authorization and `/api/sync/consent` succeed. Every transition advances an in-memory epoch before asynchronous work continues. Queue reads, writes, delayed flushes, and uploads recheck the exact consent/epoch before and after asynchronous boundaries; revocation cancels the pending flush, aborts in-flight fetches, waits out stale operations, clears the queue durably, and deletes retained server workout/mutation keys. Offline revocation remains pending for retry. Notification permission does not imply workout-upload consent.
...
The document shell and executable application runtime have separate ownership: `index.html` contains markup/styles and ordered owned script references, while `app.js` owns the local-first UI/runtime behavior. Domain modules remain separate (`prescription-engine.js`, `guided-mesocycle.js`, `rest-completion-controller.js`, and `backup-contract.js`) and load before `app.js`. Static contract tests read an explicit combined contract view without restoring runtime JavaScript to HTML. Root/`www` parity, the service-worker allowlist, and the deployment CSP all verify `app.js` as a first-class public asset.
...
- The executable app runtime is external, so Vercel `script-src` is exactly `'self'` and has no line-ending-sensitive inline hash. The deployment contract rejects executable inline script bodies, obsolete hashes, missing `app.js` ownership, or root/packaged parity drift.
