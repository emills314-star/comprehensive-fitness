...
Mesocycle construction is portfolio-first and priority-ordered: normalized evidence creates traceable pools; program slots derive direct-volume/frequency requirements; major muscles and high-value compounds are selected first; the distributor places work across named sessions while enforcing 18 working sets and two exercises per muscle per day; smaller/accessory work uses remaining capacity. The review separately aggregates direct sets, 0.35-weighted meaningful secondary contribution, zero-credit incidental involvement, fatigue, duration, overlap, frequency, and capacity conflicts. Templates consume these planned sessions. Candidate selection and regeneration rerun the same deterministic constraints rather than randomizing around warnings.
