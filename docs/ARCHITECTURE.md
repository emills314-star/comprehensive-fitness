...
Programming families are derived accounting, not a destructive taxonomy migration. For one exercise, family-level hypertrophy volume selects the strongest qualifying relationship once; local-fatigue exposure remains additive, is calculated without intermediate rounding, and is rounded only at the final display boundary. Stable persistent IDs are never reused. Logged workout facts remain immutable; version rollback means loading the prior public taxonomy export and atomically recomputing derived totals, not rewriting historical records. The checked-in 2.0.0 digest fixture encodes all 1,756 historical mapping identities. **NEEDS REVIEW:** its current test allowlist was not advanced for later 3.0.0 rule mappings, so the stable-ID gate fails after the historical comparisons and must be repaired before release.
...
- `npm test` / `npm run test:public`: dynamically discover tracked public `scripts/test*.js` tests while excluding personal-data validation. `npm run test:private` is an explicit local-only gate that requires ignored personal artifacts; `npm run test:all-local` combines both.
