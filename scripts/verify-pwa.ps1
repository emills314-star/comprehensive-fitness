...
$sensitiveDirectoryNames = @("private-personal-data", "personal_fitness_data", "personal-fitness-data", "backups")
$sensitiveExtensions = @(".db", ".sqlite", ".sqlite3", ".bak", ".backup")
foreach ($publicRoot in $publicRoots) {
  ...
  if ($sensitiveFiles) { throw "Sensitive files found in public payload $publicRoot`: $($sensitiveFiles.FullName -join ', ')" }
}
