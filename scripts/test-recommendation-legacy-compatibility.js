...
test("all four repository-known version pairs read while generation remains latest-only", () => {
  ...
  assert.equal(ENGINE_VERSION, "3.3.6", "the identity-namespace resolver patch must remain traceable");
  assert.equal(current.engineVersion, "3.3.6", "the current serialized recommendation behavior requires a traceable patch engine version");
});
...
test("engine 3.3.6 can generate an exact canonical target without changing the persisted schema", () => {
  ...
  assert.equal(currentExact.engineVersion, "3.3.6");
...
test("mixed history loads and appends without rewriting legacy records or IDs", () => {
  ...
  assert.deepEqual(loaded[2], currentExact, "legacy records must not prevent the engine 3.3.6 exact-target record from loading");
