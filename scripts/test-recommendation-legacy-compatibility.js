...
const PAIRS = [
  ...
  { snapshot: "1.4.0", prescription: "2.4.0", engine: "3.4.0" }
];
...
function legacySnapshot(pair, options = {}) {
  ...
  if (pair.snapshot !== "1.4.0") delete snapshot.standardGuideline;
...
test("all five repository-known version pairs read while generation remains latest-only", () => {
