...
assert.match(views, /Research standard[\s\S]*Today’s guideline[\s\S]*Saved default/);
for (const label of ["Within standard", "Below standard", "Above standard"]) assert.match(views, new RegExp(label));
assert.match(views, /function renderStandardWorkloadControls[\s\S]*const standard = snapshot\.standardGuideline \|\| snapshot\.basePrescription;[\s\S]*standard\.workingSets\.max/, "guideline comparison must bind the versioned standard before rendering");
assert.doesNotMatch(views, /data-standard-save-template checked/, "future-template saving must be unchecked by default");
assert.match(workout, /completedWorking[\s\S]*incompleteWorking[\s\S]*warmups[\s\S]*rebuiltIncomplete/);
