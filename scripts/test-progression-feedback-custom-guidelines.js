...
assert.match(views, /Exercise defaults[\s\S]*Research[\s\S]*Saved[\s\S]*Working sets[\s\S]*Rep range[\s\S]*Target RPE[\s\S]*Working rest[\s\S]*Warm-up rest/);
...
assert.match(views, /data-set-default-field="type"[\s\S]*data-set-default-field="rep-min"[\s\S]*data-set-default-field="target-rpe"[\s\S]*data-set-default-field="rest"/, "the nested editor must expose per-set type, rep, RPE, and rest controls");
assert.doesNotMatch(views, /data-standard-save-template checked/, "future-template saving must be unchecked by default");
assert.match(workout, /function applyExerciseDefaultTargets[\s\S]*completedWorking[\s\S]*incompleteWorking[\s\S]*targetRpe[\s\S]*targetRestSeconds[\s\S]*User-saved exercise defaults/);
