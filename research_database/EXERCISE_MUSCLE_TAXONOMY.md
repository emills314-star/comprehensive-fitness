...
Logged dates, loads, repetitions, RPE, completion, and exercise identity remain immutable. The shared historical family ledger derives versioned family totals without persistence: canonical muscle ownership controls family assignment, one exercise contributes at most once per family, direct credit wins over fractional credit, fatigue remains additive and separate, and only final totals are rounded. Canonical relationships must carry one common taxonomy version. A custom exercise may coexist only through explicit `personal-muscle-mapping/1.0.0` Primary/Secondary muscle evidence; name-based inference is not accepted as taxonomy. Missing, mixed, or unresolved provenance returns `blocked_unverifiable_provenance` and no family dose; an empty history window is a known zero. A taxonomy or personal-mapping change and rollback recalculate from immutable source records, require no application-data migration, and are regression-tested by switching evidence and returning to the original projection.
